...
void * worker_main (void * arg)
{
	...
	if (params->hardware_event == INSTR) {
		evt_fd = setup_perf_counter(PERF_TYPE_HARDWARE, PERF_COUNT_HW_INSTRUCTIONS);
		event = "INSTR";
	} else if (params->hardware_event == L1MISS) {
		evt_fd = setup_perf_counter(PERF_TYPE_HW_CACHE, (PERF_COUNT_HW_CACHE_L1D) |
														(PERF_COUNT_HW_CACHE_OP_READ << 8) |
														(PERF_COUNT_HW_CACHE_RESULT_MISS << 16));
		event = "L1MISS";
	} else if (params->hardware_event == LLCMISS) {
		evt_fd = setup_perf_counter(PERF_TYPE_HW_CACHE, (PERF_COUNT_HW_CACHE_LL) |
														(PERF_COUNT_HW_CACHE_OP_READ << 8) |
														(PERF_COUNT_HW_CACHE_RESULT_MISS << 16));
		event = "LLCMISS";
	} else {
		printf("ERROR: Invalid hardware event in worker thread\n");
	}
	...
	while (!params->worker_done) {
		...
		event_ctr1 = read_perf_counter(evt_fd);
		...
		event_ctr2 = read_perf_counter(evt_fd);
...
void handle_connection(int conn_socket, struct connection_params conn_params)
{
	...
	if (conn_params.hardware_event == INSTR) {
		evt_fd = setup_perf_counter(PERF_TYPE_HARDWARE, PERF_COUNT_HW_INSTRUCTIONS);
	} else if (conn_params.hardware_event == L1MISS) {
		evt_fd = setup_perf_counter(PERF_TYPE_HW_CACHE, (PERF_COUNT_HW_CACHE_L1D) |
														(PERF_COUNT_HW_CACHE_OP_READ << 8) |
														(PERF_COUNT_HW_CACHE_RESULT_MISS << 16));
	} else if (conn_params.hardware_event == LLCMISS) {
		evt_fd = setup_perf_counter(PERF_TYPE_HW_CACHE, (PERF_COUNT_HW_CACHE_LL) |
														(PERF_COUNT_HW_CACHE_OP_READ << 8) |
														(PERF_COUNT_HW_CACHE_RESULT_MISS << 16));
	} else {
	...
	ioctl(evt_fd, PERF_EVENT_IOC_ENABLE);
	do {
	...
	while((opt = getopt(argc, argv, "q:w:p:h:")) != -1) {
		...
		case 'h':
			if (strcmp(optarg, "INSTR") == 0) {
				conn_params.hardware_event = INSTR;
				printf("INFO: Setting hardware event = INSTR\n");
			} else if (strcmp(optarg, "L1MISS") == 0) {
				conn_params.hardware_event = L1MISS;
				printf("INFO: Setting hardware event = L1MISS\n");
			} else if (strcmp(optarg, "LLCMISS") == 0) {
				conn_params.hardware_event = LLCMISS;
				printf("INFO: Setting hardware event = LLCMISS\n");
			} else {
				printf("ERROR: Invalid hardware event\n");
			}
			break;
		default: /* '?' */
